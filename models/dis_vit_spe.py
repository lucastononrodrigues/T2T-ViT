...
class AttentionPerf(nn.Module):
    def __init__(self, dim, in_dim, head_cnt=1, kernel_ratio=0.5, dp1=0.1,spe=None):
        ...
        if self.disentangled:
            self.pos_key_proj=nn.Linear(dim,self.emb,bias=True)
            self.pos_query_proj=nn.Linear(dim,self.emb,bias=True)
            
    def attn(self, x,pos):
        B, N, C = x.shape
        k_pos=self.pos_key_proj(pos)
        #k_pos=k_pos.repeat(B,1,1,1)
        k_pos=k_pos.reshape(1,N,self.head_cnt,C//self.head_cnt)
        k_pos=k_pos.permute(0,2,1,3) # B N h d -> B h N d
        q_pos=self.pos_query_proj(pos)
        #q_pos=q_pos.repeat(B,1,1,1)
        q_pos=q_pos.reshape(1,N,self.head_cnt,C//self.head_cnt)
        q_pos=q_pos.permute(0,2,1,3)
        device = x.device
...
@register_model
def vip_dis_cifar100(pretrained=False,**kwargs):
